...
from app.forms import UserCreateForm, SessionCreateForm, PostCreateForm, CommentCreateForm, CommentUpdateForm, \
    PostUpdateForm, ProfileUpdateForm
...
class LogOut(restful.Resource):
    # @auth.login_required
    def post(self):
        # print "user email is",g.user.email
        # usr = User.query.filter_by(id = g.user.id).first()
        # print "user is", user
        # if user is None:
        #   return 'User not found!', 404
        # if user.id != g.user.id:
        #   return 'You cannot logout the user!',403
        # logout_user(user)
        # logout_user(usr)
        return 200
...
class PostView(restful.Resource):
    ...
    def delete(self, id):  # delete article
        ...
        print "article to be deleted is", post, "the owner of it", post.user_id, "current user", g.user.id
        if post == None:
            abort(404)
    ...
    @auth.login_required
    def put(self, id):  # edit article
        form = PostUpdateForm()
        if not form.validate_on_submit():
            return form.errors, 422
        post = Post.query.filter_by(id=id).first()  # request.args.get('postid')
        # currentUser =
        print "the post to be edited is: ", post, "post owner is:", post.user_id, "global userid", g.user.id
        if post.user_id != g.user.id:
...
class CommentListView(restful.Resource):
    def get(self, postid):
        ...
        print "posts' comments", postComments
        if postComments == []:
            abort(404)
