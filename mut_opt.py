...
def main(argv):
    ...
    #########--------------------------------------------HOM----------------------------------------------------#######
    #merging CT and CS
    CTS(inputfile[:-4], templatename, queryfile)
def NewDir(i):
    print('in NewDir')
            ...
            #                print(strin)
             ...
             #                   print('transition is ', s.text)
              ...
              #                          print('tarn name ', ss, 'changes to', s.text)
                                        treex.write(inp + 'MUT_CN' + str(k) + '_' + str(
                                            ii) + '.xml')
                                        i += 1
               ...
               #                              Change_dir(MyName, 'no')
                                        ...
                                        os.remove(inp + 'MUT_CN' + str(k) + '_' + str(
                                            ii) + '.xml')
                                        # print MyName,'it is deleted'
                                else:
                                    os.remove(
                                        inp + 'MUT_CN' + str(k) + '_' + str(ii) + '.xml')
    ...
    for t in root.findall('template'):
        ...
        if t.find('name').text==tem:
            for ii in range(len(r)):
                for k in range(len(r2)):
                    strin = 'transition['+str(k)+']'
                    MyName=inp+'MUT_CTS'+str(k)+'_'+str(ii)+'.xml'
                    tree.write(MyName)
                    treex = ET.parse(MyName)
                    rootx = treex.getroot()
                    for t in rootx.findall('template'):
                        ...
                        if t.find('name').text==tem:
                            ...
                            for s in tra:
                                target = s.iter('target')
                                source = s.iter('source')
                                print "Before: target", target,"source", source
                                if target != source: # not a loop edge
                                    t=target.attrib['ref']
                                    target.attrib['ref'] = source.attrib['ref']
                                    source.attrib['ref']= t
